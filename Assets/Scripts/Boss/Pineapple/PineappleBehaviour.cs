...
public class PineappleBehaviour : MonoBehaviour {
	...
	// Distance that the pineapple hides underground
	...
	public float appearedTime = 1.5f;
	
    // The rate at which the boss spawns enemies
	public float enemySpawnRate = 0.25f;
	...
	void Start () {
        ...
        // Figure out where the ground is so it can appear just above it afterwards
        ...
        while (true) {
			switch(currentState) {
				case PineappleState.SELECTING_BEHAVIOUR:
					...
					if (spawnEnemy) {
						appearX = AdjustAppearX(SpawnEnemies_AppearX());
					} else {
